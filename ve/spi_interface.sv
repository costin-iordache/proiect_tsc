...
interface spi_intf(input logic clk,reset);
  ...
  property p_sclk_active_only_when_ss_active;
    @(posedge clk) disable iff (reset) (ss == 1'b0) |-> (!isunknown(sclk));
  endproperty

  property p_mosi_valid_only_when_ss_active;
    @(posedge clk) disable iff (reset) (ss == 1'b0) |-> (!isunknown(mosi));
  endproperty

  property p_miso_valid_only_when_ss_active;
    @(posedge clk) disable iff (reset) (ss == 1'b0) |-> (!isunknown(miso));
  endproperty
